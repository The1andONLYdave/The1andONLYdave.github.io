...
ApplicationMain.preload = function() {
	...
	ApplicationMain.loadFile("assets/art/area02_level_tiles2.png");
	ApplicationMain.loadFile("assets/art/bullet.png");
	ApplicationMain.loadFile("assets/art/coinspin.png");
	ApplicationMain.loadFile("assets/art/lizardhead3.png");
	ApplicationMain.loadFile("assets/art/lizgibs.png");
	ApplicationMain.loadFile("assets/art/lurkgibs.png");
	ApplicationMain.loadFile("assets/art/lurkmonsta.png");
	ApplicationMain.loadFile("assets/art/pointer.png");
	ApplicationMain.loadFile("assets/art/simples_pimples.png");
	ApplicationMain.loadFile("assets/art/spikegibs.png");
	ApplicationMain.loadFile("assets/art/spikemonsta.png");
	ApplicationMain.loadBinary("assets/data/coins.csv");
	ApplicationMain.loadBinary("assets/data/lurkcoords.csv");
	ApplicationMain.loadBinary("assets/data/monstacoords.csv");
	ApplicationMain.loadBinary("assets/data/monstacoords.txt");
	ApplicationMain.loadBinary("assets/levels/mapCSV_Group1_Ladders.csv");
	ApplicationMain.loadBinary("assets/levels/mapCSV_Group1_Map1.csv");
	ApplicationMain.loadBinary("assets/levels/mapCSV_Group1_Map1back.csv");
	ApplicationMain.loadSound("assets/music/ScrollingSpace.mp3");
	ApplicationMain.loadSound("assets/music/ScrollingSpace.ogg");
	ApplicationMain.loadSound("assets/sounds/badshoot.wav");
	ApplicationMain.loadSound("assets/sounds/coin.wav");
	ApplicationMain.loadSound("assets/sounds/death.wav");
	ApplicationMain.loadSound("assets/sounds/jump.wav");
	ApplicationMain.loadSound("assets/sounds/menu.wav");
	ApplicationMain.loadSound("assets/sounds/mondead2.wav");
	ApplicationMain.loadSound("assets/sounds/monhurt2.wav");
	ApplicationMain.loadSound("assets/sounds/shoot2.wav");
...
var Bullet = function() {
	...
	this.loadGraphic("assets/art/bullet.png",false);
	this.set_exists(false);
};
...
Bullet.prototype = $extend(flixel.FlxSprite.prototype,{
	update: function() {
		if(!this.alive && this.animation.get_finished()) this.set_exists(false);
		if(this.getScreenXY().x < -64 || this.getScreenXY().x > flixel.FlxG.width + 64) this.kill(); else if(this.touching != 0) this.kill(); else flixel.FlxSprite.prototype.update.call(this);
	}
	,shoot: function(X,Y,VelocityX,VelocityY) {
		flixel.FlxSprite.prototype.reset.call(this,X,Y);
		this.set_solid(true);
		this.velocity.set_x(VelocityX);
		this.velocity.set_y(VelocityY);
	}
	,angleshoot: function(X,Y,Speed,Target) {
		flixel.FlxSprite.prototype.reset.call(this,X,Y);
		this.set_solid(true);
		var rangle = Math.atan2(Target.y - (this.y + this.get_height() / 2),Target.x - (this.x + this.get_width() / 2));
		this.velocity.set_x(Math.cos(rangle) * Speed);
		this.velocity.set_y(Math.sin(rangle) * Speed);
	}
	,__class__: Bullet
});
var Coin = function(X,Y) {
	...
	this.loadGraphic("assets/art/coinspin.png",true);
	this.animation.add("spinning",[0,1,2,3,4,5],10,true);
	this.animation.play("spinning");
};
$hxClasses["Coin"] = Coin;
Coin.__name__ = ["Coin"];
...
Coin.prototype = $extend(flixel.FlxSprite.prototype,{
	kill: function() {
		...
		flixel.FlxG.sound.play("assets/sounds/coin" + Reg.SoundExtension,3,false);
		Reg.score++;
	}
	,__class__: Coin
});
...
var EnemyTemplate = function(X,Y,ThePlayer) {
	flixel.FlxSprite.call(this,X,Y);
	this._startx = X;
	this._starty = Y;
...
var Enemy = function(X,Y,ThePlayer,Gibs) {
	EnemyTemplate.call(this,X,Y,ThePlayer);
	this._spawntimer = 0;
	this.loadGraphic("assets/art/spikemonsta.png",true);
	this.animation.add("walking",[0,1],10,true);
	this.animation.add("idle",[0]);
	this.drag.set_x(420);
	this.drag.set_y(420);
	this.acceleration.set_y(0);
	this.maxVelocity.set_x(60);
	this.maxVelocity.set_y(60);
	this.health = 1;
	this._gibs = Gibs;
};
$hxClasses["Enemy"] = Enemy;
Enemy.__name__ = ["Enemy"];
Enemy.__super__ = EnemyTemplate;
Enemy.prototype = $extend(EnemyTemplate.prototype,{
	_gibs: null
	,_spawntimer: null
	,update: function() {
		if(!this.alive) {
			this._spawntimer += flixel.FlxG.elapsed;
			if(this._spawntimer >= 30) this.reset(this._startx,this._starty);
			return;
		}
		...
		if(distancesquared < 65000) {
			...
			if(this._player.y < this.y) this.acceleration.set_y(-this.drag.y); else if(this._player.y > this.y) this.acceleration.set_y(this.drag.y);
		}
		if(this.velocity.x == 0 && this.velocity.y == 0) this.animation.play("idle"); else this.animation.play("walking");
		EnemyTemplate.prototype.update.call(this);
	}
	,reset: function(X,Y) {
		EnemyTemplate.prototype.reset.call(this,X,Y);
		this.health = 1;
		this._spawntimer = 0;
	}
	,hurt: function(Damage) {
		if(this.facing == 16) this.velocity.set_x(this.drag.x * 4); else if(this.facing == 1) this.velocity.set_x(-this.drag.x * 4);
		flixel.effects.FlxFlicker.flicker(this,0.5,0.04,true,true,null,null);
		this;
		flixel.FlxG.sound.play("assets/sounds/monhurt2" + Reg.SoundExtension,1,false);
		EnemyTemplate.prototype.hurt.call(this,Damage);
	}
	,kill: function() {
		...
		if(this._gibs != null) {
			...
			flixel.FlxG.sound.play("assets/sounds/mondead2" + Reg.SoundExtension,1,false);
		}
		EnemyTemplate.prototype.kill.call(this);
		this.set_exists(true);
...
var Lurker = function(X,Y,ThePlayer,Bullets) {
	EnemyTemplate.call(this,X,Y,ThePlayer);
	this._spawntimer = 0;
	this._burntimer = 0;
	...
	this._cooldown = 0;
	this.loadGraphic("assets/art/lurkmonsta.png",true,16,17);
	this.animation.add("walking",[0,1],18,true);
	this.animation.add("burning",[2,3],18,true);
	this.animation.add("wrecked",[4,5],18,true);
	this.animation.add("idle",[0]);
	this.drag.set_x(270);
	this.drag.set_y(420);
	this.acceleration.set_y(300);
	this.maxVelocity.set_x(30);
	this.maxVelocity.set_y(60);
	this.health = 2;
	this.offset.set_x(3);
	this.set_width(10);
};
$hxClasses["Lurker"] = Lurker;
Lurker.__name__ = ["Lurker"];
Lurker.__super__ = EnemyTemplate;
Lurker.prototype = $extend(EnemyTemplate.prototype,{
	_spawntimer: null
	,_burntimer: null
	,_playdeathsound: null
	,_bullets: null
	,_cooldown: null
	,update: function() {
		if(this.touching == 4096) {
			if(this.health <= 0 && this._playdeathsound) {
				flixel.FlxG.sound.play("assets/sounds/mondead2" + Reg.SoundExtension,1,false);
				this._playdeathsound = false;
			}
		}
		if(this.velocity.x == 0 && this.velocity.y == 0) this.animation.play("idle"); else if(this.health < 2) {
			if(this.velocity.y == 0) this.animation.play("wrecked"); else this.animation.play("burning");
		} else this.animation.play("walking");
		if(this.health > 0) {
			if(this.velocity.y == 0) this.acceleration.set_y(-this.acceleration.y);
			if(this.x != this._startx) this.acceleration.set_x(this._startx - this.x);
			var xdistance = this._player.x - this.x;
			var ydistance = this._player.y - this.y;
			var distancesquared = xdistance * xdistance + ydistance * ydistance;
			if(distancesquared < 45000) this.shoot(this._player);
		}
		if(this.health <= 0) {
			this.maxVelocity.set_y(240);
			this.acceleration.set_y(900);
			this.velocity.set_x(0);
			this._burntimer += flixel.FlxG.elapsed;
			if(this._burntimer >= 5) {
				EnemyTemplate.prototype.kill.call(this);
				this.set_x(-10);
				this.set_y(-10);
				this.set_visible(false);
			...
			this._spawntimer += flixel.FlxG.elapsed;
			if(this._spawntimer >= 45) this.reset(this._startx,this._starty);
		}
	...
	,hurt: function(Damage) {
		if(this.x > this._player.x) this.velocity.set_x(this.drag.x * 4); else this.velocity.set_x(-this.drag.x * 4);
		flixel.effects.FlxFlicker.flicker(this,0.5,0.04,true,true,null,null);
		this;
		flixel.FlxG.sound.play("assets/sounds/monhurt2" + Reg.SoundExtension,1,false);
		this.health -= 1;
	...
	,shoot: function(P) {
		...
		if(this._cooldown > 1) {
			...
			if(P.x < this.x) bulletX -= Math.floor(bullet.width - 8); else bulletX += Math.floor(this.get_width() - 8);
			bullet.angleshoot(bulletX,bulletY,100,flixel.util.FlxPoint.get(P.x,P.y));
			flixel.FlxG.sound.play("assets/sounds/badshoot" + Reg.SoundExtension,1,false);
			this._cooldown = 0;
		}
	}
	,kill: function() {
...
MenuState.prototype = $extend(flixel.FlxState.prototype,{
	...
	,create: function() {
		...
		flixel.FlxG.game._state.set_bgColor(-15723500);
		this._text1 = new flixel.text.FlxText(-220,flixel.FlxG.height / 4,320,"Project");
		this._text1.set_moves(true);
		this._text1.set_size(40);
		this._text1.set_color(41704);
		this._text1.antialiasing = true;
		this._text1.velocity.set_x(600);
		this.add(this._text1);
		this._text2 = new flixel.text.FlxText(flixel.FlxG.width - 50,flixel.FlxG.height / 2.5,320,"Montezumasrevenge");
		this._text2.set_moves(true);
		this._text2.set_size(this._text1.get_size());
		this._text2.set_color(this._text1.color);
		this._text2.antialiasing = this._text1.antialiasing;
		this._text2.velocity.set_x(-600);
		this.add(this._text2);
		this._text3 = new flixel.text.FlxText(flixel.FlxG.width * 2 / 3,flixel.FlxG.height * 2 / 3,150,"Play");
		this._text4 = new flixel.text.FlxText(flixel.FlxG.width * 2 / 3,flixel.FlxG.height * 2 / 3 + 30,150,"Visit our Webpage");
		this._text5 = new flixel.text.FlxText(flixel.FlxG.width * 2 / 3,flixel.FlxG.height * 2 / 3 + 60,150,"Visit flixel.org");
		this._text3.set_color(this._text4.set_color(this._text5.set_color(-1442798872)));
		this._text3.set_size(this._text4.set_size(this._text5.set_size(16)));
		this._text3.antialiasing = this._text4.antialiasing = this._text5.antialiasing = true;
		this.add(this._text3);
		this.add(this._text4);
		this.add(this._text5);
		this._pointer = new flixel.FlxSprite();
		this._pointer.loadGraphic("assets/art/pointer.png");
		this._pointer.set_x(this._text3.x - this._pointer.get_width() - 10);
	...
	,update: function() {
		if(flixel.FlxG.mouse.wheel != 0) {
			...
			_g.set_zoom(_g.zoom + flixel.FlxG.mouse.wheel / 10);
		}
		if(this._text1.x > flixel.FlxG.width / 5) this._text1.velocity.set_x(0);
		if(this._text2.x < flixel.FlxG.width / 2.5) this._text2.velocity.set_x(0);
		var _g1 = this._option;
		switch(_g1) {
		case 0:
			this._pointer.set_y(this._text3.y);
			break;
		case 1:
			this._pointer.set_y(this._text4.y);
			break;
		case 2:
			this._pointer.set_y(this._text5.y);
			break;
		}
		if(flixel.FlxG.keys.checkStatus(38,flixel.FlxG.keys.justPressed.checkStatus)) {
			this._option = (this._option + 3 - 1) % 3;
			flixel.FlxG.sound.play("assets/sounds/menu" + Reg.SoundExtension,1,false);
		}
		if(flixel.FlxG.keys.checkStatus(40,flixel.FlxG.keys.justPressed.checkStatus)) {
			this._option = (this._option + 3 + 1) % 3;
			flixel.FlxG.sound.play("assets/sounds/menu" + Reg.SoundExtension,1,false);
		}
		if(flixel.FlxG.keys.checkKeyStatus(["SPACE","ENTER","C"],2)) {
			var _g2 = this._option;
			switch(_g2) {
			case 0:
				flixel.FlxG.cameras.fade(-6907801,1,false,$bind(this,this.startGame));
				flixel.FlxG.sound.play("assets/sounds/coin" + Reg.SoundExtension,1,false);
				break;
			case 1:
				flixel.FlxG.openURL("http://the1andonlydave.github.com",null);
				break;
			case 2:
				flixel.FlxG.openURL("http://flixel.org",null);
				break;
			}
		}
...
PlayState.prototype = $extend(flixel.FlxState.prototype,{
	...
	,create: function() {
		...
		this.map.allowCollisions = 4369;
		this.background = new flixel.tile.FlxTilemap();
		this.ladders = new flixel.tile.FlxTilemap();
		this._restart = false;
		this.add(this.background.loadMap(openfl.Assets.getText("assets/levels/mapCSV_Group1_Map1back.csv"),"assets/art/simples_pimples.png",16,16,0));
		this.background.scrollFactor.set_x(this.background.scrollFactor.set_y(.5));
		this.add(this.map.loadMap(openfl.Assets.getText("assets/levels/mapCSV_Group1_Map1.csv"),"assets/art/simples_pimples.png",16,16));
		this.add(this.ladders.loadMap(openfl.Assets.getText("assets/levels/mapCSV_Group1_Ladders.csv"),"assets/art/simples_pimples.png",16,16));
		flixel.FlxG.camera.setBounds(0,0,this.map.get_width(),this.map.get_height());
		flixel.FlxG.worldBounds.set(0,0,this.map.get_width(),this.map.get_height());
		this._gibs = new flixel.effects.particles.FlxEmitter();
		this._gibs.setXSpeed(-150,150);
		this._gibs.setYSpeed(-200,0);
		this._gibs.setRotation(-720,720);
		this._gibs.makeParticles("assets/art/lizgibs.png",25,16,true,.5);
		this._mongibs = new flixel.effects.particles.FlxEmitter();
		this._mongibs.setXSpeed(-150,150);
		this._mongibs.setYSpeed(-200,0);
		this._mongibs.setRotation(-720,720);
		this._mongibs.makeParticles("assets/art/spikegibs.png",25,16,true,.5);
		this._bullets = new flixel.group.FlxGroup();
		this._bullets.set_maxSize(4);
		this._badbullets = new flixel.group.FlxGroup();
		this.add(this.player = new Player(112,92,this,this._gibs,this._bullets));
		flixel.FlxG.camera.follow(this.player,1);
		this._enemies = new flixel.group.FlxGroup();
		this.placeMonsters(openfl.Assets.getText("assets/data/monstacoords.csv"),Enemy);
		this.placeMonsters(openfl.Assets.getText("assets/data/lurkcoords.csv"),Lurker);
		this._coins = new flixel.group.FlxGroup();
		this.placeCoins(openfl.Assets.getText("assets/data/coins.csv"),Coin);
		...
		this._score = new flixel.text.FlxText(0,0,flixel.FlxG.width);
		this._score.setFormat(null,16,-256,"center",2,1252379);
		this._score.scrollFactor.set(0,0);
		this.add(this._score);
		this._text1 = new flixel.text.FlxText(0,30,flixel.FlxG.width,"Press R to Restart");
		this._text1.setFormat(null,40,-65536,"center",2,-16777216);
		this._text1.set_visible(false);
		this._text1.antialiasing = true;
		this._text1.scrollFactor.set(0,0);
		this.add(this._text1);
		flixel.FlxG.sound.playMusic("assets/music/ScrollingSpace.ogg",0.5);
	}
	,update: function() {
		flixel.FlxG.overlap(this.player,this.map,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this._enemies,this.map,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this._gibs,this.map,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this._bullets,this.map,null,flixel.FlxObject.separate);
		flixel.FlxG.overlap(this._badbullets,this.map,null,flixel.FlxObject.separate);
		flixel.FlxState.prototype.update.call(this);
		this._score.set_text("$" + Std.string(Reg.score));
		if(!this.player.alive) {
			this._text1.set_visible(true);
			if(flixel.FlxG.keys.checkStatus(82,flixel.FlxG.keys.justPressed.checkStatus)) this._restart = true;
		}
		flixel.FlxG.overlap(this.player,this._enemies,$bind(this,this.hitPlayer));
		flixel.FlxG.overlap(this._bullets,this._enemies,$bind(this,this.hitmonster));
		flixel.FlxG.overlap(this.player,this._coins,$bind(this,this.collectCoin));
		flixel.FlxG.overlap(this.player,this._badbullets,$bind(this,this.hitPlayer));
		if(this._restart) flixel.FlxG.switchState(new PlayState());
	}
	,collectCoin: function(P,C) {
		C.kill();
	}
	,hitPlayer: function(P,Monster) {
		if(js.Boot.__instanceof(Monster,Bullet)) Monster.kill();
		if(Monster.health > 0) P.hurt(1);
	}
	,hitmonster: function(Blt,Monster) {
	...
	,placeMonsters: function(MonsterData,Monster) {
		var coords;
		...
		while(_g1 < _g) {
			...
			if(Monster == Enemy) this._enemies.add(new Enemy(Std.parseInt(coords[0]),Std.parseInt(coords[1]),this.player,this._mongibs)); else if(Monster == Lurker) this._enemies.add(new Lurker(Std.parseInt(coords[0]),Std.parseInt(coords[1]),this.player,this._badbullets));
		}
	}
	,placeCoins: function(CoinData,Sparkle) {
		var coords;
		...
		while(_g1 < _g) {
			var j = _g1++;
			coords = entities[j].split(",");
			if(Sparkle == Coin) this._coins.add(new Coin(Std.parseInt(coords[0]),Std.parseInt(coords[1])));
		}
	}
	,__class__: PlayState
});
var Player = function(X,Y,Parent,Gibs,Bullets) {
	...
	flixel.FlxSprite.call(this,X,Y);
	this._bullets = Bullets;
	this.loadGraphic("assets/art/lizardhead3.png",true,16,20);
	this.animation.add("walking",[0,1,2,3],12,true);
	this.animation.add("idle",[3]);
	this.animation.add("jump",[2]);
	this.drag.set(720,720);
	this.maxVelocity.set(90,250);
	this.acceleration.set_y(620);
	this.setSize(12,16);
	this.offset.set(3,4);
	this._cooldown = 0.4;
	this._gibs = Gibs;
	this._parent = Parent;
	this._jumpKeys = ["C","K","SPACE"];
};
...
Player.prototype = $extend(flixel.FlxSprite.prototype,{
	...
	,update: function() {
		...
		if(this.climbing) this.acceleration.set_y(0); else this.acceleration.set_y(620);
		...
		this.jump();
		if(this._jumpTime < 0) this.climb();
		if(flixel.FlxG.keys.checkKeyStatus(["X","J"],1)) this.shoot();
		if(this.velocity.x > 0 || this.velocity.x < 0) this.animation.play("walking"); else if(this.velocity.x == 0) this.animation.play("idle");
		if(this.velocity.y < 0) this.animation.play("jump");
		this._cooldown += flixel.FlxG.elapsed;
		if(this.x <= 0) this.set_x(0);
		if(this.x + this.get_width() > this._parent.map.get_width()) this.set_x(this._parent.map.get_width() - this.get_width());
		this._xgridleft = (this.x + 3) / 16 | 0;
		this._xgridright = Std["int"]((this.x + this.get_width() - 3) / 16);
		this._ygrid = Std["int"]((this.y + this.get_height() - 1) / 16);
		if(this._parent.ladders.getTile(this._xgridleft,this._ygrid) > 0 && this._parent.ladders.getTile(this._xgridright,this._ygrid) > 0) this._onLadder = true; else {
			this._onLadder = false;
			this.climbing = false;
		}
		if((this.touching & 4096) > 0 && !flixel.FlxG.keys.checkKeyStatus(this._jumpKeys,1)) {
			this._jumpTime = -1;
			this._timesJumped = 0;
		}
	...
	,climb: function() {
		if(flixel.FlxG.keys.checkKeyStatus(["UP","W"],1)) {
			if(this._onLadder) {
				this.climbing = true;
				this._timesJumped = 0;
			}
			if(this.climbing && this._parent.ladders.getTile(this._xgridleft,this._ygrid - 1) > 0) this.velocity.set_y(-90);
		} else if(flixel.FlxG.keys.checkKeyStatus(["DOWN","S"],1)) {
			if(this._onLadder) {
				this.climbing = true;
				this._timesJumped = 0;
			}
			if(this.climbing) this.velocity.set_y(90);
		}
	}
	,jump: function() {
		if(flixel.FlxG.keys.checkKeyStatus(this._jumpKeys,2)) {
			if(this.velocity.y == 0 || this._timesJumped < 2) {
				flixel.FlxG.sound.play("assets/sounds/jump" + Reg.SoundExtension,1,false);
				this._timesJumped++;
				this._jumpTime = 0;
				this._onLadder = false;
			}
		}
		if(flixel.FlxG.keys.checkKeyStatus(this._jumpKeys,1) && this._jumpTime >= 0) {
			this.climbing = false;
			this._jumpTime += flixel.FlxG.elapsed;
			if(this._jumpTime > 0.25) this._jumpTime = -1; else if(this._jumpTime > 0) this.velocity.set_y(-0.6 * this.maxVelocity.y);
		}
	}
	,shoot: function() {
		var bulletX = Math.floor(this.x);
		...
		var bXVeloc = 0;
		var bYVeloc = 0;
		if(this._cooldown >= 0.4) {
			this._blt = js.Boot.__cast(this._bullets.recycle() , Bullet);
			if(this._blt != null) {
				if(this.flipX) {
					bulletX -= Math.floor(this._blt.get_width() - 8);
					bXVeloc = -200;
				} else {
					bulletX += Math.floor(this.get_width() - 8);
					bXVeloc = 200;
				}
				this._blt.shoot(bulletX,bulletY,bXVeloc,bYVeloc);
				flixel.FlxG.sound.play("assets/sounds/shoot2" + Reg.SoundExtension,1,false);
				this._cooldown = 0;
			}
		}
	}
	,kill: function() {
		if(!this.alive) return;
		flixel.FlxSprite.prototype.kill.call(this);
		flixel.FlxG.cameras.shake(0.005,0.35);
		flixel.FlxG.cameras.flash(-2410972,0.35);
		if(this._gibs != null) {
			this._gibs.at(this);
			this._gibs.start(true,2.80);
		}
		flixel.FlxG.sound.play("assets/sounds/death" + Reg.SoundExtension,1,false);
	}
	,__class__: Player
});
...
Lurker.HEALTH = 2;
Lurker.SPAWNTIME = 45;
Lurker.JUMP_SPEED = 60;
Lurker.BURNTIME = 5;
Lurker.GUN_DELAY = 1;
Lurker.BULLET_SPEED = 100;
MenuState.OPTIONS = 3;
MenuState.TEXT_SPEED = 600;
Player.RUN_SPEED = 90;
Player.GRAVITY = 620;
Player.JUMP_SPEED = 250;
Player.JUMPS_ALLOWED = 2;
Player.BULLET_SPEED = 200;
Player.GUN_DELAY = 0.4;
Reg.SoundExtension = ".wav";
Reg.levels = [];
Reg.level = 0;
Reg.scores = [];
Reg.score = 0;
...
flixel.FlxG.worldDivisions = 6;
flixel.FlxG.VERSION = new flixel.system.FlxVersion(3,3,5);
flixel.FlxG.elapsed = 0;
flixel.FlxG.maxElapsed = 0.1;
